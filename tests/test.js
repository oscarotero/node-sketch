    ...
    it('Should open the file without errors', () => {
        return sketch.read(__dirname + '/test.sketch').then(file => {
            ...
            describe('Must read the pages', () => {
                ...
                it('Symbols page', () => assert.strictEqual(file.pages[1], file.symbolsPage));
            });

            describe('Must read the symbols', () => {
                it('count symbols local symbols', () => assert.equal(file.symbols.length, 2));
                ...
                it('count instances', () => {
                    ...
                    assert.equal(file.pages[0].getAll('symbolInstance').length, 3);
                });
                ...
                it('get foreign master of an instance', () => {
                    const instance = file.pages[0].get('symbolInstance', 'arrow');
            ...
            describe('Must read the shared styles', () => {
                it('count layer styles', () => assert.equal(file.layerStyles.length, 2));
                it('count foreign layer styles', () => assert.equal(file.foreignLayerStyles.length, 1));
                it('first layer style name', () => assert.equal(file.layerStyles[0].name, 'style-1'));
                it('second layer style name', () => assert.equal(file.layerStyles[1].name, 'style-2'));
                it('count text styles', () => assert.equal(file.textStyles.length, 2));
                it('count foreign text styles', () => assert.equal(file.foreignTextStyles.length, 1));
                it('first text style name', () => assert.equal(file.textStyles[0].name, 'style-1'));
                it('second text style name', () => assert.equal(file.textStyles[1].name, 'style-2'));
            });
            ...
            describe('Must read the gradients library', () => {
                it('count document gradients', () => assert.equal(file.gradients.length, 1));
                it('first gradient type', () => assert.equal(file.gradients[0].gradientType, 2));
            });
            ...
            describe('Must search nodes', () => {
                ...
                it('Node.get#2', () => assert(page.get('text', text => text.name === 'Hello world')));
                it('Node.get#3', () => assert(page.get(text => text._class === 'text' && text.name === 'Hello world')));
                it('Node.getAll', () => assert(page.getAll('text', 'Hello world').length, 1));

                const text = page.get('text', 'Hello world');

                it('Node.parent', () => {
                    assert.strictEqual(text.parent, page.get('artboard', 'Insert'));
                ...
                it('Node.parent#4', () => {
                    assert.strictEqual(text.getParent(node => node._class === 'page'), file.pages[0]);
                });
            });

            describe('Must read a text style', () => {
